...
inline void gemm_ukernel_Cresident_SIMD_4x10(int mr, int nr, int kc, DTYPE *Ar, int ldA, DTYPE *Br, int ldB, DTYPE *C, int ldC, char orderC, DTYPE beta) {
// mr x nr = 4 x 10 micro-kernel and C resident in regs.
// Inputs:
//   - C stored in column-major order, with leading dimension ldC
//   - Ar packed by columns, with leading dimension mr = 4
//   - Br packed by rows, with leading dimension nr = 10

  if (kc == 0) return;
  const int MR = 4; 
  const int NR = 10; 
  int       i, j, pr, baseA = 0, baseB = 0, ldCt = MR, Amr, Bnr;
  vregister Cr0_0, Cr0_1, Cr0_2, Cr0_3, Cr0_4, Cr0_5, Cr0_6, Cr0_7, Cr0_8, Cr0_9, 
            Cr1_0, Cr1_1, Cr1_2, Cr1_3, Cr1_4, Cr1_5, Cr1_6, Cr1_7, Cr1_8, Cr1_9, 
            A0_0;
  vregister ar0, ar1, br0, br1, br2, br3, br4; 
  DTYPE zero = 0.0, Ctmp[MR * NR], *Aptr, *Bptr; 


  vinit(Cr0_0); vinit(Cr0_1); vinit(Cr0_2); vinit(Cr0_3); vinit(Cr0_4); vinit(Cr0_5); vinit(Cr0_6); vinit(Cr0_7); vinit(Cr0_8); vinit(Cr0_9); 
  vinit(Cr1_0); vinit(Cr1_1); vinit(Cr1_2); vinit(Cr1_3); vinit(Cr1_4); vinit(Cr1_5); vinit(Cr1_6); vinit(Cr1_7); vinit(Cr1_8); vinit(Cr1_9); 
  vinit(ar0); vinit(ar1); 
